#!/usr/bin/env python3
"""Validate the global XZIEL weapon catalog and Mystery Box pool."""
...
POOL = Path("assets/weapons/xziel_mystery_box_pool_v1.json")
...
def main() -> int:
    ...
    pool_ids = pool.get("weaponIds", [])
    if pool_ids != expected_pool:
        fail("Mystery Box pool is stale; regenerate it from the catalog")
    if len(pool_ids) != len(set(pool_ids)):
        fail("Mystery Box pool contains duplicate weapon IDs")
    if any(wid not in by_id for wid in pool_ids):
        fail("Mystery Box pool contains orphan weapon IDs")

    print(
        "XZIEL_WEAPON_CATALOG_OK "
        f"weapons={len(weapons)} zmSource={zm_count} "
        f"box={len(pool_ids)} nachtFirearms={len(firearm_wall_buys)}"
    )
    return 0
